...
# Redis进阶 - 如何保障MySQL和Redis的数据一致性？

## 0. 前言

直接先抛一下结论：**在满足实时性的条件下，不存在两者完全保存一致的方案，只有最终一致性方案。** 根据网上的众多解决方案，总结出 6 种，直接看目录：

![image-20221129233458742](https://zszblog.oss-cn-beijing.aliyuncs.com/zszblog/image-20221129233458742.png)

## 1 不好的方案

### 1.1 先写 MySQL，再写 Redis

![image-20221129233531099](https://zszblog.oss-cn-beijing.aliyuncs.com/zszblog/image-20221129233531099.png)
...
### 1.2 先写 Redis，再写 MySQL

![image-20221129233651886](https://zszblog.oss-cn-beijing.aliyuncs.com/zszblog/image-20221129233651886.png)

同“先写 MySQL，再写 Redis”，看图可秒懂。

### 1.3 先删除 Redis，再写 MySQL

这幅图和上面有些不一样，前面的请求 A 和 B 都是更新请求，这里的请求 A 是更新请求，**但是请求 B 是读请求，且请求 B 的读请求会回写 Redis。**

![image-20221129233801952](https://zszblog.oss-cn-beijing.aliyuncs.com/zszblog/image-20221129233801952.png)
...
### 1.4 先删除 Redis，再写 MySQL，再删除 Redis

对于“先删除 Redis，再写 MySQL”，如果要解决最后的不一致问题，其实再对 Redis 重新删除即可，**这个也是大家常说的“缓存双删”。**

![image-20221129233931661](https://zszblog.oss-cn-beijing.aliyuncs.com/zszblog/image-20221129233931661.png)
...
![image-20221129234009705](https://zszblog.oss-cn-beijing.aliyuncs.com/zszblog/image-20221129234009705.png)
...
### 1.5. 先写 MySQL，再删除 Redis

![image-20221129234057440](https://zszblog.oss-cn-beijing.aliyuncs.com/zszblog/image-20221129234057440.png)
...
![image-20221129234242330](https://zszblog.oss-cn-beijing.aliyuncs.com/zszblog/image-20221129234242330.png)
...
### 1.6 先写 MySQL，通过 Binlog，异步更新 Redis

这种方案，主要是监听 MySQL 的 Binlog，然后通过异步的方式，将数据更新到 Redis，这种方案有个前提，查询的请求，不会回写 Redis。

![image-20221129234344095](https://zszblog.oss-cn-beijing.aliyuncs.com/zszblog/image-20221129234344095.png)
...
[如何保障MySQL和Redis的数据一致性？](https://tobebetterjavaer.com/mysql/redis-shuju-yizhixing.html)