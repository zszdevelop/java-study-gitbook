...
# Spring进阶 - spring事务失效的12种场景

## 0. 前言

在某些业务场景下，如果一个请求中，需要同时写入多张表的数据。为了保证操作的原子性（要么同时成功，要么同时失败），避免数据不一致的情况，我们一般都会用到spring事务。

spring事务使用起来很简单，只需要一个简单的注解：`@Transactional`

但如果你使用不当，它也会坑你于无形。

今天我们就一起聊聊，事务失效的一些场景，说不定你已经中招了。不信，让我们一起看看。

![image-20221215193650290](https://zszblog.oss-cn-beijing.aliyuncs.com/zszblog/image-20221215193650290.png)

## 1. 事务不生效

>`@Transactional`注解的，声明式事务。主要是通过 spring aop实现。 spring aop 在一些特定环境下会失效
>
>- java访问权限 非public的时候
>- final 方法的时候
>- 未被spring管理的时候
>
>还有就是：多线程环境下、数据库表本身不支持事务、或者压根没开启事务
...
因为如果使用默认值，一旦程序抛出了Exception，事务不会回滚，这会出现很大的bug。所以，建议一般情况下，将该参数设置成：Exception或Throwabl
...
![image-20221215211520686](https://zszblog.oss-cn-beijing.aliyuncs.com/zszblog/image-20221215211520686.png)
